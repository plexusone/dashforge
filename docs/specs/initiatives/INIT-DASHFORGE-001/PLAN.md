# PLAN — DashForge Rename & DashForge Split
...
1. GitHub rename `plexusone/dashforge` → `plexusone/dashforge`; move local
...
5. **Verification gate**: ecosystem-wide grep sweep (no live
   `plexusone/dashforge` references outside historical docs), remotes checked,
...
6. Boundary inventory doc: per-package classification (dashforge-core vs
   dashforge-extract) with dependency directions.
7. Create fresh `plexusone/dashforge` (retires the redirect — depends on the
...
8. First extraction: UISpec/PageSpec types + schema generation into the new
   dashforge; dashforge consumes as a dependency. Further extraction items are
   logged in the new repo's own roadmap as they are sized.

**Exit criteria:** new dashforge builds and tests standalone; dashforge imports
...
- New dashforge repo's own initiative for the component-platform roadmap once
