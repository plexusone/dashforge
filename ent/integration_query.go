...
func (_q *IntegrationQuery) QueryAlerts() *AlertQuery {
...
func (_q *IntegrationQuery) QueryAlertChannels() *AlertChannelQuery {
...
func (_q *IntegrationQuery) WithAlerts(opts ...func(*AlertQuery)) *IntegrationQuery {
	query := (&AlertClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *IntegrationQuery) WithAlertChannels(opts ...func(*AlertChannelQuery)) *IntegrationQuery {
	query := (&AlertChannelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *IntegrationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Integration, error) {
	...
	if query := _q.withAlerts; query != nil {
		if err := _q.loadAlerts(ctx, query, nodes,
			func(n *Integration) { n.Edges.Alerts = []*Alert{} },
			func(n *Integration, e *Alert) { n.Edges.Alerts = append(n.Edges.Alerts, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAlertChannels; query != nil {
		if err := _q.loadAlertChannels(ctx, query, nodes,
			func(n *Integration) { n.Edges.AlertChannels = []*AlertChannel{} },
			func(n *Integration, e *AlertChannel) { n.Edges.AlertChannels = append(n.Edges.AlertChannels, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *IntegrationQuery) loadAlerts(ctx context.Context, query *AlertQuery, nodes []*Integration, init func(*Integration), assign func(*Integration, *Alert)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Integration)
	nids := make(map[int]map[*Integration]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(integration.AlertsTable)
		s.Join(joinT).On(s.C(alert.FieldID), joinT.C(integration.AlertsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(integration.AlertsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(integration.AlertsPrimaryKey[1]))
...
func (_q *IntegrationQuery) loadAlertChannels(ctx context.Context, query *AlertChannelQuery, nodes []*Integration, init func(*Integration), assign func(*Integration, *AlertChannel)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(alertchannel.FieldIntegrationID)
	}
	query.Where(predicate.AlertChannel(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(integration.AlertChannelsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "integration_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
