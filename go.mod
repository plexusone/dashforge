...
exclude (
	...
	// picks v1.0.0 up anyway since MVS treats it as the latest available
	// version. Exclude it until cobrautil bumps its own automemlimit
	// requirement past this break.
	github.com/KimMachineGun/automemlimit v1.0.0
	// authzed/cel-go v0.30.0 rewrote cel.NewStaticOptimizer to return
	// (*StaticOptimizer, error) instead of *StaticOptimizer. authzed/spicedb
	// v1.56.1's pkg/caveats/replacer now expects that new 2-return-value
	// signature (the inverse of the old v1.56.0-era assumption below v0.30.0
	// never got a real tag), so only v0.30.0 itself needs excluding — v0.32.0
	// has the fix and is what's actually required. If `go get -u` starts
	// complaining again, spicedb's cel-go expectation shifted again; check
	// pkg/caveats/replacer's signature before re-excluding.
...
require (
	...
	github.com/plexusone/systemforge v0.10.0
