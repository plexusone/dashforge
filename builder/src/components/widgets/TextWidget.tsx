...
export function TextWidget({ widget }: TextWidgetProps) {
  ...
  const renderContent = () => {
    if (!config.content) {
      return <span className="text-gray-400 italic">Enter text content...</span>
    }
    ...
    switch (config.format) {
      ...
      default:
        return <p className="whitespace-pre-wrap">{config.content}</p>
    }
  }

  return <div className="h-full w-full p-4 overflow-auto">{renderContent()}</div>
}
