  ...
  return (
    <div>
      <label className="block text-xs font-medium text-gray-600 mb-2">
        Dimensions
        <span className="text-gray-400 font-normal ml-1">({selected.length} selected)</span>
      </label>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              {dimension.description && (
                <div className="text-xs text-gray-500 truncate">{dimension.description}</div>
              )}
            </div>
            {dimension.primaryKey && <span className="text-xs text-yellow-600">🔑</span>}
          </button>
        ))}
        {dimensions.length === 0 && (
          <p className="text-sm text-gray-400 text-center py-2">No dimensions available</p>
        )}
      </div>
    </div>
  )
